...
# tuple list för formatera om svar för latex konvertering
...
# Uppdatera render_message funktionen
def render_message(message: str):
    # Ersätt LaTeX-symboler i hela meddelandet först
    message = replace_latex_symbols(message)
    
    # Hantera blockformler (omgivna av dubbla dollar-tecken)
    parts = re.split(r'(\$\$.*?\$\$)', message, flags=re.DOTALL)
    for part in parts:
        if part.startswith('$$') and part.endswith('$$'):
            # Rendera blockformel
            latex = part[2:-2]  # Ta bort $$ symbolerna
            st.latex(latex)
        else:
            # Hantera inline formler och text
            inline_parts = re.split(r'(\$.*?\$)', part)
            for inline_part in inline_parts:
                if inline_part.startswith('$') and inline_part.endswith('$'):
                    # Rendera inline formel
                    latex = inline_part[1:-1]  # Ta bort $ symbolerna
                    st.latex(latex)
                else:
                    # Rendera vanlig text med markdown
                    st.markdown(inline_part)


# # ---------- Function definitions for function calling ------------- #
# def generate_image(prompt: str, output_format: str = "webp", size: str = "1024x1024") -> str:
#     try:
...
# Add new function for DALL-E 3 -> used instead of stability-ai 
...
# research - google search using serper
...
# research - using tavily and retrieving images
...
# read website - using jina.ai reader
    ...
    try:
        ...
        summary = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": content[:4000]}  # Limit content to avoid token limits
            ]
        )
...
functions = [
    ...
    #             "output_format": {"type": "string", "description": "The format of the output image (default: webp)"},
    #             "size": {"type": "string", "description": "The size of the image (default: 1024x1024)"}
    #         },
    #         "required": ["prompt"]
    #     }
    # },
...
if user_prompt:
    ...
    #     with st.expander('Se genererade bilder'):
    ...
    #                 file_name=f"edith_bild_{idx + 1}.{image_info['format']}",
